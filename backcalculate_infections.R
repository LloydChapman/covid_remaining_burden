...
# - switch delay distributions to those in covidm
# - use country names or two-letter country iso codes throughout
...
# scale cumulative deaths by distribution of total deaths over that time period
...
# - correct dates for vaccination data to be end of ISO week
...
ined_deaths = fread("../ined_data/AgeSex/covid_pooled_AS_2021-06-11.csv")
...
deaths = clean_ined_death_data(ined_deaths,cols)
...
out10 = clean_COVerAGE_death_data(COVerAGE_deaths,"deaths")
...
dt_ined = construct_data_table(agegroups,deaths,pop,cols,ltc_deaths,vax,Ab_delay,ifr)
dt = construct_data_table(agegroups,out10,pop,c("cum_deaths_male","cum_deaths_female","cum_deaths_both"),ltc_deaths,vax,Ab_delay,ifr)
...
ggsave("./output/infections_by_age_non_LTC_backproj_COVerAGE.pdf",plot=p_non_ltc_backproj,width = 10,height = 8)
p_ltc_backproj = plot_infections(dt_ltc_backproj)
ggsave("./output/infections_by_age_LTC_backproj_COVerAGE.pdf",plot=p_ltc_backproj,width = 10,height = 8)
p_non_ltc_ride = plot_infections(dt_non_ltc_ride) #[country %in% c("Denmark","Norway")])
ggsave("./output/infections_by_age_non_LTC_ride_COVerAGE.pdf",plot=p_non_ltc_ride,width = 10,height = 8)
p_ltc_ride = plot_infections(dt_ltc_ride)
ggsave("./output/infections_by_age_LTC_ride_COVerAGE.pdf",plot=p_ltc_ride,width = 10,height = 8)
...
ggplot(ecdc_cases_by_age[country %in% dt[,unique(country)]],aes(x=date,y=new_cases,group=age_group,color=age_group)) +
    geom_line() +
    facet_wrap(~country)
ggsave("./output/ecdc_cases_by_age.pdf",width = 10,height = 8)
ggplot(ecdc_cases_by_age[country_code %in% c("DK","NO")],aes(x=date,y=new_cases,group=age_group,color=age_group)) +
    geom_line() +
    facet_wrap(~country)
ggsave("./output/ecdc_cases_by_age_DNK_NOR.pdf",width = 6,height = 3)
...
ggplot() +
    ...
    facet_wrap(~country)
ggsave("./output/infections_vs_obs_cases_ride_COVerAGE.pdf",width = 10,height = 8)
...
ggplot() +
    ...
    facet_wrap(~country)
ggsave("./output/infections_vs_obs_cases_by_age_ride_COVerAGE.pdf",width = 10,height = 8)
...
ggplot() +
    ...
    facet_wrap(~country)
ggsave("./output/infections_vs_obs_cases_by_age_ride_DNK_NOR_COVerAGE.pdf",width = 6,height = 3)
...
ggplot() +
    ...
    facet_wrap(~country)
ggsave("./output/cum_prop_infected_vs_seroprev_COVerAGE.pdf",width = 10,height = 8)

p_deaths = plot_deaths(dt)
ggsave("./output/deaths_by_age_COVerAGE.pdf",plot = p_deaths,width = 10,height = 8)

ggplot(dt,aes(x=date,y=cum_prop_va+cum_prop_vb,group=age_group,color=age_group)) +
    geom_line() +
    facet_wrap(~country)
ggsave("./output/vax_cov_by_age_COVerAGE.pdf",width = 10,height = 8)

save.image("../backcalculation_wip.RData")
...
print(AIC_BIC)
#    model      AIC      BIC
# 1:    m1 482724.5 483531.5
# 2:    m2 466772.5 467983.0

vrnt_prop1 = vector("list",ncountries)
vrnt_prop2 = vector("list",ncountries)
new_dt = data.table(date=dt_ride[,unique(date)]) #data.table(date = seq.Date(vrnt_data_wide[,min(date)],dates2[length(dates2)],by=1))
for (i in 1:ncountries){
    vrnt_prop1[[i]] = data.table(new_dt,predict(m1[[i]],newdata = new_dt,type = "probs"))
    vrnt_prop1[[i]][,country:=countries1[i]]
    vrnt_prop2[[i]] = data.table(new_dt,predict(m2[[i]],newdata = new_dt,type = "probs"))
    vrnt_prop2[[i]][,country:=countries1[i]]
}
...
# Use model m1 for the time being despite its higher AIC and BIC as it appears
# to be a bit more robust
# N.B. It is very wrong for Hungary, Malta and Ireland, so shouldn't be used for
# these countries

# Merge into incidence data table
inc_dt = merge(inc_dt,vrnt_prop1,by=c("country","date"),allow.cartesian=T)
...
ggplot(melt(curr_dt[country!="Greece"],measure.vars = c("pop_prop_u","pop_prop_v")),aes(x=value,y=age_group_model,alpha=variable)) +
    ...
    facet_wrap(~country)
ggsave("./output/vax_cov_pop_pyramids.pdf",width = 10,height = 8)
...
ggplot(curr_dt[country!="Greece"],aes(x=ovrl_cum_inc_hosp*1e5,y=age_group_model)) +
    ...
    facet_wrap(~country)
ggsave("./output/rem_hosps_by_age.pdf",width = 10,height = 8)
...
ggplot(ovrl_curr_dt[country!="Greece"],aes(x=cum_prop_v,y=ovrl_cum_inc_hosp*1e5)) + 
    ...
    labs(x = "Proportion fully vaccinated",y = "Maximum remaining hospitalisations/100,000 population") +
    scale_y_log10()
ggsave("./output/rem_hosps_vs_prop_full_vax_vs.pdf",width = 10,height = 8)
